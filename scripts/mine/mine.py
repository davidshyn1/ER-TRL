...
from scripts.mine.models import StatisticsNet
...
class MINE_Base(nn.Module):
    ...
    def __init__(self, input_dim, K, est_lr=2e-4, variant='unbiased', device='cpu'):
        super().__init__()
        self._T = StatisticsNet(input_dim, K, device=device)
        self._est_lr = est_lr
