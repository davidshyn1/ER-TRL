...
class StatisticsNet(nn.Module):
    ...
    def __init__(self, x_dim, z_dim, device='cpu'):
        ...
        self._layers.append(nn.Linear(x_dim+z_dim, 100, device=device))    # x_dim+z_dim. 512
        self._layers.append(nn.Linear(100, 100, device=device))            # 512, 512
        self._out_layer = nn.Linear(100, 1, device=device)                 # 512, 1
        self._initialize_weights()
        self.device=device
    ...
    def forward(self, x, z):
        ...
        x = x + 0.3 * torch.randn_like(x, device=self.device)
        for hid_layer in self._layers:
            x = F.elu(hid_layer(x))
            x = x + 0.5 * torch.randn_like(x, device=self.device)
