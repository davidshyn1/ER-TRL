    ...
    def __init__(
        ...
        if std is None:
            if self.std_architecture == "shared":
                ...
                self.last_fc_log_std = self.last_fc_log_std.to(self.device)
...
class TanhGaussianRAPPolicy(TorchStochasticPolicy):
    ...
    def __init__(
        ...
        self.history_embed_fcs = nn.Sequential(
            nn.Linear(self.input_size * self.stack_size, embedding_hidden_size, bias=False),
            # nn.BatchNorm1d(embedding_hidden_size),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(embedding_hidden_size, embedding_dim)
        ).to(self.device)
        self.history_embedding_params = self.history_embed_fcs.parameters()
        
        self.single_embed_fcs = nn.Sequential(
            nn.Linear(self.input_size, embedding_hidden_size, bias=False),
            # nn.BatchNorm1d(embedding_hidden_size),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(embedding_hidden_size, embedding_dim)
        ).to(self.device)
        ...
        self.policy_fcs = nn.Sequential(
            ...
            nn.Linear(embedding_dim*2, policy_hidden_size),
            nn.LeakyReLU(0.2, inplace=True),
        ).to(self.device)
        self.policy_params.append({'params': self.policy_fcs.parameters()})
        self.policy_mean = nn.Linear(policy_hidden_size, action_dim).to(self.device)
        ...
        if policy_std is None:
            self.policy_fc_log_std = nn.Linear(policy_hidden_size, action_dim).to(self.device)
        ...
        self.residual_fcs = nn.Sequential(
            # nn.LeakyReLU(0.2, inplace=False),
            nn.Linear(embedding_dim, residual_hidden_size),
            nn.LeakyReLU(0.2, inplace=True),
        ).to(self.device)
        self.residual_params.append({'params': self.residual_fcs.parameters()})
        self.residual_mean = nn.Linear(residual_hidden_size, action_dim).to(self.device)        
...
class TanhGaussianPolicyWithEmbedding(TorchStochasticPolicy):
    ...
    def __init__(
        ...
        self.policy_fcs = nn.Sequential(
            nn.LeakyReLU(0.2, inplace=False),
            nn.Linear(embedding_dim, policy_hidden_size),
            nn.LeakyReLU(0.2, inplace=True),
        ).to(self.device)
        # self.policy_params.append({'params': self.policy_fcs.parameters()})
        self.policy_mean = nn.Linear(policy_hidden_size, action_dim).to(self.device)
        ...
        # self.policy_fc1 = nn.Linear(embedding_dim, policy_hidden_size).to(self.device)
        ...
        # self.policy_fc2 = nn.Linear(policy_hidden_size, action_dim).to(self.device)
